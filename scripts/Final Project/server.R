...
tab2_columns$Rotten.Tomatoes <- gsub("%", "",
                                     as.character(tab2_columns$Rotten.Tomatoes))
...
server <- function(input, output) {
  ...
  output$chart2 <- renderPlotly({
    ...
    p <- ggplot(tab2_columns, aes_string(
  ...
  output$chart3 <- renderPlotly({
    ...
    p <- ggplot(stock_plot) +
      geom_point(
        mapping = aes_string(x = "year", y = input$chart3_y_var),
