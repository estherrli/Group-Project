...
intro_page <- tabPanel(
  ...
  h1("Introduction"),
  p("In the year of 2020, COVID-19 caused a nationwide lockdown
    that led people to stay at home without access to the outside world. With
    this extended time at home, many people turned to streaming platforms such
    as Netflix for entertainment. Because of this widened use of Netflix, we
    decided to explore and learn more about the platform that we have all been
    using extensively during the past few months. Therefore, for our final
    project, we decided to analyze trends in datasets
      containing information on Netflix."),
  p("Our first major question was: How have
      the different movie genres on Netflix changed over time, and what can this
      tell us about trends in the movie industry? To answer this,
      we found a dataset called 'Netflix Titles', which includes data on
      directors, casts, genres, duration, and release years for Netflix movies.
      Using this dataset, we were able to create an interactive bar chart to
      display patterns in the different movie genres on Netflix."),
  p("Our second major question was: what are some main components that
      determine the quality of a movie and therefore gain a large audience?
      To find the answer to this, we decided to explore ratings of movies and
      how specific variables affected them. To do so, we found a dataset,
      called ‘Movies On Streaming Platforms’, that compared individual movies’
      age group, runtime, and genre against IMDb ratings and Rotten Tomato
      ratings. Using a scatterplot, we plotted the variables against the ratings
      to find if any patterns or correlations were revealed."),
  p("Our third major question was how Netflix's value changed and can it be
      attributed to overall media consumption? Our second graph analyzes data
      regarding Netlfix stock prices since it first went public in 2002. The
      data contains information on daily opening, closing, maximum, minimum,
      volume of stock traded, and closing prices adjusted with split and
      dividend multipliers. Our interactive scatter plot allows you to analyze
      each of these variables individually from year to year and
      illustrate patterns."),
  tags$img(src = "https://specials-images.forbesimg.com/imageserve/5e9ddacf8c2caa0006e6f7a5/960x0.jpg?fit=scale")
)
...
page_one <- tabPanel(
  tags$h4("Netflix Movie Genres"),
  h1("Netflix Movie Genre Exploration"),
  sidebarLayout(
    sidebarPanel(p(chart1_genre), p(chart1_color)),
    mainPanel(p(plotlyOutput("chart1", height = "700px")),
              p("This interactive bar chart displays the number of movies added
              to Netflix each year for a chosen genre. The chosen genre can be
              altered using the first widget. The second widget allows users to
              change the color of the bars. Using this chart, we can explore the
              total counts of each movie genre each year -- for example, we can
              see that the most Children & Family movies were added in 2019,
              while the most Stand-Up Comedies were
              added in 2018.")
              )
  ),
